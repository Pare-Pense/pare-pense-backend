...
export class AnaliseGastosService {
   ...
   async processarAnalisePosGasto(idUsuario: string, 
    ...
    novaDespesa: { id: string; valor: number; categoria: Categoria }) {
        ...
        try {
            ...
            const despesasDaCategoria = await despesaService.recuperarDespesasAll(
                idUsuario,
                undefined, 
            ...
            if (history.length > 0) {
                ...
                if (zScoreRes.ok) {
                    ...
                    if (zScoreDados.z_score > 2.5) {
                        ...
                        console.log(`Notificação de anomalia (Z-Score: ${zScoreDados.z_score}) criada para o usuário ${idUsuario}.`);
                    }
                } else {
                    console.error(`Erro na API de Z-Score: ${zScoreRes.statusText}`);
                }
            }
        } catch (error) {
        ...
        try {
            ...
            const todasDespesasMensais = await despesaService.recuperarDespesasAll(
                idUsuario,
                'mensal', 
                undefined 
            );
            ...
            if (gastosPorDia.size < 8) {
                console.log(`Usuário ${idUsuario} tem apenas ${gastosPorDia.size} dias de dados.`);
                return; 
            }
            ...
            if (diaDoEstouro <= diaFinal && diaDoEstouro > dataAtual.getDate()) {
                
                const notificacaoRecente = await this.db.notificacao.findFirst({
                    where: {
                        idUsuario,
                        titulo: 'Alerta de Orçamento',
                        createdAt: { gte: new Date(dataAtual.setDate(dataAtual.getDate() - 2)) } 
                    }
                });
                ...
                if (!notificacaoRecente) {
                    ...
                    //console.log(`notificacao ${idUsuario}.`);
                }
            }
